  ...
  return (
    <main className="w-full flex items-center justify-center overflow-hidden">
      <div className="relative w-[calc(100vw-40px)] h-[calc(100vw-40px)] md:w-[450px] md:h-[450px] flex items-center justify-center">
        ...
        <GlowingOrbitPath radius={180} glowColor="purple" animationDelay={0.5} />

        {/* Central Icon */}
        <div
          className="relative w-20 h-20 rounded-full bg-gray-900/80 backdrop-blur-sm flex items-center justify-center text-white font-bold text-lg cursor-pointer hover:scale-110 transition-transform duration-300"
        ...
        {skillsConfig.map((skill) => {
          const angle = time * skill.speed + skill.phaseShift;
          return <OrbitingSkill key={skill.id} config={skill} angle={angle} />;
        })}
      </div>
    </main>
  );
